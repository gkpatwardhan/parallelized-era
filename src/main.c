...
__attribute__((noinline)) void grid_fusion(
		Observation *observations, size_t observations_sz,
		unsigned char *uncmp_data, size_t uncmp_data_sz,
		double local_av_x, double local_av_y,
		double input_av_x, double input_av_y,
		unsigned int local_x_dim, unsigned int local_y_dim,
		unsigned int input_x_dim, unsigned int input_y_dim,
		double local_cell_size,
		double input_cell_size) {
#if (defined(HPVM) || defined(HPVM_RECV_PIPELINE)) && false
	void *Section_Inner = __hetero_section_begin();
	void *T4 = __hetero_task_begin(12, observations, observations_sz, uncmp_data, uncmp_data_sz,
			local_av_x, local_av_y, input_av_x, input_av_y, 
			local_x_dim, local_y_dim, input_x_dim, input_y_dim, local_cell_size, input_cell_size, 
			1, localMap, localMapSize, "gridFusion_task");

	// HPVM: Nested inside this task is a call to fuseIntoLocal() function. This function has a for-loop
	// inside of it.
	// So, this T4 can be split into two sub-tasks where one sub-task just contains the call to fuseIntoLocal()
	// and the second sub-task contains the logging stuff that follows the call to fuseIntoLocal().
	// The rough structure of fuseIntoLocal() is as follows:
	...
	fuseIntoLocal(observations, observations_sz, uncmp_data, uncmp_data_sz, local_av_x, local_av_y, input_av_x, input_av_y,
			local_x_dim, local_y_dim, input_x_dim, input_y_dim, local_cell_size, input_cell_size);
...
#if defined(WRITE_FUSED_MAPS) || defined(WRITE_ASCII_MAP)

#if (defined(HPVM) || defined(HPVM_RECV_PIPELINE)) && false
	void *T5 = __hetero_task_begin(12, localMap, localMapSize, inputMap, inputMapSize, local_av_x, local_av_y, input_av_x, input_av_y,
			local_x_dim, local_y_dim, input_x_dim, input_y_dim, local_cell_size, input_cell_size,
			1, localMap, localMapSize, "logging_task");
#endif
...
#if (defined(HPVM) || defined(HPVM_RECV_PIPELINE)) && true
	void *T4_Wrapper1 = __hetero_task_begin(12, uncmp_data, uncmp_data_sz, local_av_x, local_av_y, 
						input_av_x, input_av_y, local_x_dim, local_y_dim, input_x_dim, input_y_dim,
						local_cell_size, input_cell_size, observations, observations_sz,
						2, uncmp_data, uncmp_data_sz, observations, observations_sz, 
						"gridFusion_task_Wrapper1");
#endif

	// HPVM: Nested inside this task is a for-loop that can be parallelized
	grid_fusion(uncmp_data, uncmp_data_sz, observations, observations_sz, local_av_x, local_av_y, input_av_x, input_av_y, 
			local_x_dim, local_y_dim, input_x_dim, input_y_dim, local_cell_size, input_cell_size);
...
void *receive_and_fuse_maps_impl(Observation *observations /*=observations -> global*/, size_t observations_sz /*=2*/)
{
	...
	while (1)
	{
		...
		if (valread == 8)
		{ // Read in 8-bytes; so we read a unsigned long (plus metadata)
			...
			Costmap2D* receivedData = (Costmap2D*)uncmp_data;
			...
			void *LaunchInner = __hetero_launch((void *)fuse_maps, 41,
					...
					// End variables used by do_recv_pipeline
					// Variables used by fuseIntoLocal
					observations[curr_obs].master_costmap.av_x, 
					observations[curr_obs].master_costmap.av_y,
					receivedData->av_x, 
					receivedData->av_y,
					observations[curr_obs].master_costmap.x_dim, 
					observations[curr_obs].master_costmap.y_dim,
					receivedData->x_dim,  
					receivedData->y_dim,
					observations[curr_obs].master_costmap.cell_size,
					receivedData->cell_size,
					// Outputs
			...
			fuse_maps(n_recvd_in,
					...
					// End variables used by do_recv_pipeline
					// Variables used by fuseIntoLocal
					observations[curr_obs].master_costmap.av_x, observations[curr_obs].master_costmap.av_y,
					receivedData->av_x, receivedData->av_y,
					observations[curr_obs].master_costmap.x_dim, observations[curr_obs].master_costmap.y_dim,
					receivedData->x_dim,  receivedData->y_dim,
					observations[curr_obs].master_costmap.cell_size,
					receivedData->cell_size);
#endif
...
void process_lidar_to_occgrid(lidar_inputs_t *lidar_inputs, size_t lidarin_sz /*=sizeof( * lidar_inputs)*/,
		...
		double AVxyzw, 
		bool* rolling_window, size_t rolling_window_sz,
...
)
{
	...
	*timer_sequentialize = 1;
	...
	// CloudToOccgrid
	{
		...
		void *T1_cloudToOccgrid_Task = __hetero_task_begin(10, observationVal, observations_sz, lidar_inputs, lidarin_sz,
				...
				max_obstacle_height, raytrace_range, 
				size_x, size_y, resolution, 
				timer_sequentialize, timer_sequentialize_sz,
				2, observationVal, observations_sz, timer_sequentialize, timer_sequentialize_sz,
				"initCostmap_task");

		// HPVM: Technically, this task has a small for-loop that has the potential to be parallelized.
		// The for-loop is inside the function initCostmap() (which is called in this task).
		// initCostmap() has the following strucutre:
		//    At the start, the function initializes the values of observationVal without a for-loop. This is
		//      the bulk of the function
		//    Towards the end, there is a one line for-loop that iterates over an array inside observationVal
		//      and assigns each value in the array a constant value. This can be parallelized.
		// As one can see though, to parallelize that inner for-loop would mean there have to be some changes
		// below; specifically, the 3 lines where we initialize robot_x, robot_y, and robot_z would have to
		// awag and we will have to pass these values in from the call to process_lidar_to_occgrid
#endif
		{
			*timer_sequentialize = 1;
#ifdef INT_TIME
		...
		// HPVM: updateOrigin() calls the function copyMapRegion() which has a 3 for-loops, two of which
		// can definitely be parallelized. The third is a double for-loop (i.e. one for-loop inside another)
		// however that one may not be straight forward to parallelize. Technically, each round of the double
		// for-loop should be able to go in parallel, however the issue is: the code inside the for-loop uses
		// this index variable (sm_index & dm_index) which is used to index into the arrays used inside the
		// for-loop. These index variables are incremented in each iterator of the inner for-loop and are
		// thus used by the next iteration of the inner for-loop.
		// While this may seem like a dependency in the for-loop iterations, we should use a simple equation
		// to calculate the appropirate sm_index and dm_index values (maybe:
		//    sm_index = original value of sm_index before for lopp (i*region_y_dim + j)
		//    dm_index = original value of dm_index before for lopp (i*region_y_dim + j) )
		// The REAL issue with trying to parallelize the internal for-loops is the if-statement below. HPVM 
		// doesn't yet support conditionals in internal node. I can't think of a way to "separate" the if-statement
		// and the call to updateOrigin() aside from just moving that if-statement into the for-loop.
#endif
...
#if (defined(HPVM) || defined(HPVM_PROCESS_LIDAR_INTERNAL)) && true
		void *T3_cloudToOccgrid_Task = __hetero_task_begin(4, observationVal, observations_sz, lidar_inputs, lidarin_sz,
				AVxyzw, lidar_data_size, 1, observationVal, observations_sz, "updateBounds_task");
...
#endif
			updateBounds(observationVal, observations_sz, lidar_inputs, lidarin_sz, AVxyzw, lidar_data_size);
	...
	*timer_sequentialize = 3;
	...
	// HPVM: There were no for-loops inside LZ4_compress_default(). Most of the code regions in LZ4_compress_default() are also
	// dependent on each other so there doesn't seem to be any parallelism inside this task.
#endif
		...
		)
{
	...
	void *Section = __hetero_section_begin();
	void *T1 = __hetero_task_begin(18, lidar_inputs, lidarin_sz, n_cmp_bytes, n_cmp_bytes_sz,
	...
	process_lidar_to_occgrid(lidar_inputs, lidarin_sz, observationVal, observations_sz,
			...
			lmap_count_cp, lmap_count_cp_sz,
			// Args for cloudToOccgrid
			AVxyzw, rolling_window, rolling_window_sz, min_obstacle_height, 
			max_obstacle_height, raytrace_range, 
			size_x, size_y, resolution, 
			lidar_data_size,
...
#if (defined(HPVM) || defined(HPVM_PROCESS_LIDAR)) && true
			void *lidarDAG = __hetero_launch((void *)lidar_root, 33, &lidar_inputs, sizeof(lidar_inputs_t),
					...
					size_y,
					resolution,
					lidar_inputs.data_size / sizeof(float),
