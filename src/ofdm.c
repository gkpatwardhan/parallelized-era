...
void decode_signal(unsigned num_inputs, fx_pt constellation[DECODE_IN_SIZE_MAX], unsigned *num_outputs, uint8_t *output_data) // hls::stream< ap_uint<1> > &output_data  )
{
  ...
  unsigned num_out_bits = num_inputs / 2; // for BPSK_1_2
  {
    ...
    sdr_decode(true, &ofdm, &frame, bit /*input_data*/, &n_res_char, output_data); // HPVM: This function has some tiny little for-loop everywhere which can be parallelized, however, everything else in the function doesn't have parallelism
