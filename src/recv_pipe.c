...
do_rcv_fft_work(unsigned num_fft_frames, fx_pt1 fft_ar_r[FRAME_EQ_IN_MAX_SIZE], fx_pt1 fft_ar_i[FRAME_EQ_IN_MAX_SIZE])
{
	...
	{    // scope for jidx
		...
		for (unsigned i = 0; i < num_fft_frames /*SYNC_L_OUT_MAX_SIZE/64*/; i++)
		{ // This is the "spin" to invoke the FFT
		...
		{ // This is the "spin" to invoke the FFT
			...
			else
			{
				for (unsigned j = 0; j < 64; j++)
				{
					...
					fft_in_imag[j] = (float)cimagf(fftSample);
					DO_LIMITS_ANALYSIS(
							if (fft_in_real[j] < min_input) { min_input = fft_in_real[j]; } if (fft_in_real[j] > max_input) { max_input = fft_in_real[j]; } if (fft_in_imag[j] < min_input) { min_input = fft_in_imag[j]; } if (fft_in_imag[j] > max_input) { max_input = fft_in_imag[j]; });
				}
			}
...
__attribute__((noinline)) void pre_process_input_for_compute(float* recvd_in_imag, size_t recvd_in_imag_sz,
	...
	for (int i = 0; i < num_recvd_vals; i++) { // HPVM: This for-loop can be parallelized
#if defined(HPVM) && true
		__hetero_parallel_loop(1, 4, input_data_arg, input_data_arg_sz,
				recvd_in_real, recvd_in_real_sz,
				recvd_in_imag, recvd_in_imag_sz,
				num_recvd_vals,
				1, input_data_arg, input_data_arg_sz, "process_input_to_compute_loop");
#endif
		...
		fx_pt *d_sync_long_out_frames_arg /*= d_sync_long_out_frames -> global*/, size_t d_sync_long_out_frames_arg_sz /*= SYNC_L_OUT_MAX_SIZE*/)
{
	...
	DEBUG(printf("In do_recv_pipeline: num_received_vals = %u\n", num_recvd_vals); fflush(stdout));
#endif
	pre_process_input_for_compute(recvd_in_real, recvd_in_real_sz, recvd_in_imag, recvd_in_imag_sz, 
		...
		fx_pt *d_sync_long_out_frames_arg /*= d_sync_long_out_frames -> global*/, size_t d_sync_long_out_frames_arg_sz /*= SYNC_L_OUT_MAX_SIZE*/)
{
	...
	// HPVM: There is a single for-loop inside complex_conjugate() which can be easily parallelized
#endif
	...
	// HPVM: complex_multiply() only contains a for-loop which can be easily parallellized
#endif
	{
		unsigned num_del16_vals = num_inputs + 16;
	...
	// HPVM: ffirc() only contains a for-loop which can be easily parallellized
#endif
	{
		unsigned num_del16_vals = num_inputs + 16;
	...
	// HPVM: complex_to_magnitude() only contains a for-loop which can be easily parallellized
#endif
	{
		unsigned num_del16_vals = num_inputs + 16;
	...
	// HPVM: complex_to_mag_squared() only contains a for-loop which can be easily parallellized
#endif
	{
	...
	// HPVM: ffir() only contains a for-loop which can be easily parallellized
#endif
	{
	...
	// HPVM: division() only contains a for-loop which can be easily parallellized
#endif
	{
#ifdef INT_TIME
	...
	// HPVM: do_rcv_fft_work() only contains 3 for-loop which can be parallellized
#endif
	...
	// HPVM: There are 7 for-loops inside this task. 4 of them is inside gr_equalize() and 2 of them can be
	// easily parallelized (the other two aren't too hard, but require some thinking on how the values of the
	// counter variables should be calculated). The other two for-loops are below and both are easy to
	// parallelize
#endif
...

	{
		...
		{ // HPVM: This for-loop can be eaisly parallelized
			equalized_bits[ii] = 0;
		}
	...
	// HPVM: decode_signal() has 3 for-loops which can be relatively easily parallelized
#endif
