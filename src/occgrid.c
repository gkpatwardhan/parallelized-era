...
void fuseIntoLocal(Costmap2D *theLocal, Costmap2D *theInput)
{
  ...
  { // TODO: HPVM: This loop can be parallellized
                  ...
                  /*,char def_val*/)
{
  ...
  for (int j = 0; j < cell_y_dim; j++)
  {
    for (int i = 0; i < cell_x_dim; i++)
    {
      ...
      if (g1_index >= cell_x_dim * cell_y_dim)
        return;
      if (g2_index >= cell_x_dim * cell_y_dim)
        return;
...
gettimeofday(&ocgr_ryFS_total_start, NULL);
  ...
  // HPVM: This for-loop is a little intriguging.
  // So the central goal, so to speak, of this for-loop update the value of obs_ptr->master_costmap.costmap[index].
  // Thus, to decide if we can parallelize this for-loop, we answers to 3 questions:
  //    1. Does the index value change based on results of the previous iteration?
  //        => No. The index value varies per iteration based on the value of i. While other variables are used as well
  //          to calculate the value of the index, the other variables are constant over the for-loop (i.e. they variables
  //          don't change value other course of the entire for-loop)
  //    2. Can two iterators modify the same index value?
  //        => I don't have a concrete answer for this. We will have to actually test out if the values are ever the same.
  //           However, just by skimming the calculations for the index, it appears that index values can repeat so we may
  //           not be able to parallelize this for-loop.
  //    3. Does the value assigned to an index depend on results of the previous iteration?
  //        => No. The value assigned to any index is always CMV_FREE_SPACE
  // Conclusion: We will have to test out to see if index values repeat. At the moment, it seems more likely that this for-loop
  // can't support parallelization.
...
void raytraceLine(Observation *obs_ptr, unsigned int x0, unsigned int y0, unsigned int x1, unsigned int y1, unsigned int max_length)
{ // TODO: default parameter for max_length is UNIT_MAX; define UNIT_MAX
